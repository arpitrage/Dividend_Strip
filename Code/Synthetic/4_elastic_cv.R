...
wkdir <- "/Users/agupta011/Dropbox/Research/Infrastructure/JFfinal/Synthetic Results/"
...
for (i in 1:nrow(fund.categories) ) {
  ...
  # Store predictions from models
  {
    ...
    fund.subset.predictions2.ols = fund.subset %>%
      select(Fund.ID, Age.Quarter, Vintage.Quarter, predictions, price.bond.feb)
    ...
    fund.subset.predictions2 = fund.subset %>%
      select(Fund.ID, Age.Quarter, Vintage.Quarter, predictions, price.bond.feb, net.cf.distribution.rescale)
  ...
  r2_pen = rbind( r2_pen.2, r2_pen.7)
  
  model = rbind("2 factor", "full factors")
