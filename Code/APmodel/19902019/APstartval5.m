...
% AP Model 
%
% This code is the fifth step in a series of files to run to produce good
% starting values for the full market price of risk estimation. This file
% uses only moments on natural resources and value stocks to pin down a  
% subset of MPR coefficients. It takes as given the the MPR coefficients 
% estimated form the first, second, third, and fourth steps.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
global LL LL1 LL2 LL3 counts
counts = 0;

% load MPR estimates from previous four steps and hold these constant in
...
L0(I_divgr_reit==1)              =LL2(1);
L0(I_divgr_infra==1)             =LL2(2);
...
L0(I_divgr_small==1)              = LL3(1);
L0(I_divgr_growth==1)             = LL3(2);
...
% x_init(1) = L0(find(I_divgr_NR==1));
% L1(find(I_divgr_NR==1),:) = ((cc_NR-aa_NR(1:15)*L1(1:15,:))/aa_NR(16));
% num_NR=length(find(L1(find(I_divgr_NR==1),:)));
% x_init(3:3+num_NR-1) = L1(find(I_divgr_NR==1),find(L1(find(I_divgr_NR==1),:)));
% % value
...
% x_init(2) = L0(find(I_divgr_value==1));
% L1(find(I_divgr_value==1),:)=((cc_value-aa_value(1:17)*L1(1:17,:))/aa_value(18));
% num_value=length(find(L1(find(I_divgr_value==1),:)));
% x_init(3+num_NR:3+num_NR+num_value-1) = L1(find(I_divgr_value==1),find(L1(find(I_divgr_value==1),:)));
...
options = optimset('DiffMinChange',.01,'TolX',0.01,'TolFun',0.01,'MaxIter',300,'MaxFunEval',300);
[x,fval,exitflag,output] = fminsearch('solvenul_startval5',x_init,options,N,Psi,Sig,I_pi,I_gdp,I_y1,I_yspr,I_pdm,I_divgrm,I_pd_reit,I_divgr_reit,I_pd_infra,I_divgr_infra,I_pd_small,I_divgr_small,I_pd_growth,I_divgr_growth,I_pd_NR,I_divgr_NR,I_pd_value,I_divgr_value,striphorizon,y0nom_1,y0nom_20,x0,pi0,X2,tau,yielddata,T,eps2,A0m,k1m,r0_m,mu_m,A0_reit,k1_reit,r0_reit,mu_reit,A0_infra,k1_infra,r0_infra,mu_infra,A0_small,k1_small,r0_small,mu_small,A0_growth,k1_growth,r0_growth,mu_growth,PDm_strip_8q_data,sharestrip_8q_data,A0_NR,k1_NR,r0_NR,mu_NR,A0_value,k1_value,r0_value,mu_value,taureal,yielddatareal);
save x_guess_5 x
