function obj = solvenul_startval5(x,N,Psi,Sig,I_pi,I_gdp,I_y1,I_yspr,I_pdm,I_divgrm,I_pd_reit,I_divgr_reit,I_pd_infra,I_divgr_infra,I_pd_small,I_divgr_small,I_pd_growth,I_divgr_growth,I_pd_NR,I_divgr_NR,I_pd_value,I_divgr_value,striphorizon,y0nom_1,y0nom_20,x0,pi0,X2,tau,yielddata,T,eps2,A0m,k1m,r0_m,mu_m,A0_reit,k1_reit,r0_reit,mu_reit,A0_infra,k1_infra,r0_infra,mu_infra,A0_small,k1_small,r0_small,mu_small,A0_growth,k1_growth,r0_growth,mu_growth,PDm_strip_8q_data,sharestrip_8q_data,A0_NR,k1_NR,r0_NR,mu_NR,A0_value,k1_value,r0_value,mu_value,taureal,yielddatareal)
global counts LL LL1 LL2 LL3
...
FF_NR=[];
FF_new = 400*(L0(find(I_divgr_NR==1))  - (((r0_NR + pi0 - y0nom_1 +0.5*(aa_NR)*(aa_NR)')-aa_NR(1:15)*L0(1:15))/aa_NR(16))); FF = [FF FF_new];FF_NR=[FF_NR FF_new];                                     FF = [FF FF_new];FF_NR=[FF_NR FF_new];    
...
% Forcing the real term structure to stay below the nominal one by at least ynrdiflongmin%
ynrdiflongmin = 1.5;
FF_new    = (tuningp*abs(min(-400*Api(200)/200+400*A(200)/200-ynrdiflongmin,0))).^2;
FF_real   = FF_new; FF        = [FF FF_new];
FF_new    = (tuningp*abs(min(-400*Api(400)/400+400*A(400)/400-ynrdiflongmin,0))).^2;
FF_real2   = FF_new; FF        = [FF FF_new];
FF_new    = (tuningp*abs(min(-400*Api(800)/800+400*A(800)/800-ynrdiflongmin,0))).^2;
FF_real3   = FF_new; FF        = [FF FF_new];
FF_new    = (tuningp*abs(min(-400*Api(1200)/1200+400*A(1200)/1200-ynrdiflongmin,0))).^2;
FF_real4   = FF_new; FF        = [FF FF_new];
FF_new    = (tuningp*abs(min(-400*Api(2000)/2000+400*A(2000)/2000-ynrdiflongmin,0))).^2;
