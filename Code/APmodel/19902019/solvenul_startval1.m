...
% Forcing the nominal bond yield to stay above nominal GDP growth in the long-run
...
if tuningp>0
    ...
    % Forcing the real yield to stay above real GDP growth in the long-run
...
FF_gooddeal = 100*(exp(max(std(mnom)-0.8,0))^2 -1);
FF      = [FF FF_gooddeal];
FF_keepLsmall = 10*sum(exp(max(sqrt(sum(L.^2))-1.5,0)).^2 -1);
FF      = [FF FF_keepLsmall];
if mod(counts,100) == 0
