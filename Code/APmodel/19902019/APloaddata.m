...
Tstart     = 173; % start sample in 1990.Q1 (1=1947.Q1)
...
% disp(['mean ann. inflation rate = ',num2str(400*pi0),' with ann. volatility  = ',num2str(100*sqrt(4)*std(inflation))])
% disp(['mean rpc gdp gr rate = ',num2str(400*x0),' with ann. volatility  = ',num2str(100*sqrt(4)*std(rpcgdpgr))])
...
X2(:,pd_reitpos)      = pd_reit;
X2(:,divgr_reitpos)   = divgr_reit; 
X2(:,pd_infrapos)     = pd_infra;
X2(:,divgr_infrapos)  = divgr_infra;
X2(:,pd_smallpos)     = pd_small;
X2(:,divgr_smallpos)  = divgr_small;
X2(:,pd_growthpos)    = pd_growth;
X2(:,divgr_growthpos) = divgr_growth;
X2(:,pd_NRpos)        = pd_NR;
X2(:,divgr_NRpos)     = divgr_NR;
X2(:,pd_valuepos)    = pd_value;
X2(:,divgr_valuepos) = divgr_value;
...
I_pd_reit     = I(:,pd_reitpos); 
I_divgr_reit  = I(:,divgr_reitpos);   
I_pd_infra    = I(:,pd_infrapos); 
I_divgr_infra = I(:,divgr_infrapos);
I_pd_small    = I(:,pd_smallpos); 
I_divgr_small = I(:,divgr_smallpos);
I_pd_growth   = I(:,pd_growthpos); 
I_divgr_growth= I(:,divgr_growthpos);
I_pd_NR       = I(:,pd_NRpos); 
I_divgr_NR    = I(:,divgr_NRpos);
I_pd_value    = I(:,pd_valuepos); 
I_divgr_value = I(:,divgr_valuepos);
